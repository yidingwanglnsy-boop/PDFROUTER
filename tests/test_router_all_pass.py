...
class TestPdfRouter(unittest.TestCase):
    ...
    @patch("pdf_router.pdf_router.open")
    @patch("pdf_router.pdf_router.open_pdfium_document")
    def test_extract_features_text_pdf(self, mock_open_pdfium, mock_file_open,
                                       mock_metadata, mock_cid, mock_avg_chars,
                                       mock_image_ratio, mock_classify, mock_page_size,
                                       mock_page_count):
        """测试纯文本PDF特征提取"""
        # Mock文件读取
        mock_file_open.return_value.__enter__.return_value.read.return_value = b"%PDF-1.4 mock content"

        # Mock PDF文档对象
        mock_pdf_doc = MagicMock()
        mock_open_pdfium.return_value = mock_pdf_doc

        # Mock返回值
        mock_page_count.return_value = 10
        mock_page_size.return_value = (595, 842)  # A4比例
        mock_classify.return_value = "text"
        mock_image_ratio.return_value = 0.05  # 低图片占比
        mock_avg_chars.return_value = 520  # 高文本密度
        ...
        self.assertAlmostEqual(features["aspect_ratio"], 595/842, places=4)
        self.assertEqual(features["pdf_type"], "text")
        self.assertEqual(features["image_coverage_ratio"], 0.05)
        self.assertEqual(features["avg_chars_per_page"], 520)
        ...
        # 验证PDF文档被关闭
        from mineru.utils.pdfium_guard import close_pdfium_document
        # close_pdfium_document应该被调用一次
        # 这里就不严格验证了，主要验证特征提取逻辑
    ...
    @patch("pdf_router.pdf_router.open")
    @patch("pdf_router.pdf_router.open_pdfium_document")
    def test_ppt_converted_detection(self, mock_open_pdfium, mock_file_open,
                                     mock_metadata, mock_cid, mock_avg_chars,
                                     mock_image_ratio, mock_classify, mock_page_size,
                                     mock_page_count):
        """测试PPT转PDF识别"""
        ...
        mock_classify.return_value = "text"
        mock_image_ratio.return_value = 0.72  # 高图片占比
        mock_avg_chars.return_value = 85  # 低文本密度
        mock_cid.return_value = False
        mock_metadata.return_value = {"producer": "Microsoft PowerPoint", "creator": "PowerPoint"}
    ...
    @patch("pdf_router.pdf_router.open")
    @patch("pdf_router.pdf_router.open_pdfium_document")
    def test_scan_pdf_detection(self, mock_open_pdfium, mock_file_open,
                                mock_metadata, mock_cid, mock_avg_chars,
                                mock_image_ratio, mock_classify, mock_page_size,
                                mock_page_count):
        """测试扫描版PDF识别"""
...
if __name__ == "__main__":
    # 运行所有测试
    print("=" * 60)
    print("运行PDF路由算子单元测试（全Mock版本，无需真实PDF）...")
    print("=" * 60)
